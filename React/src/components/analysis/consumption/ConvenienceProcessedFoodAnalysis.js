...
import AnalysisImg from '../img/consumption/convenience_processed_food.png';

export default function ConvenienceProcessedFoodAnalysis() {
    const explanation = "유의확률은 0.01 이하(F = 7.984)로 경로 중 한 가지 이상이 유효할 것이라는 것을 확인 할 수 있었다. (VIF는 0.1 이상 10 미만으로 다중공선성이 없는 것으로 확인되었다.) 다음으로 각 경로에 유의성을 확인한 결과 신규확진자(t = 2.103, p<0.05)에서 편의점 식품 음료 등 가공에 미치는 영향이 유효한 것으로 확인되었다. 유의한 변수에 대한 비표준화계수를 확인한 결과, 신규확진자(0.0042)는 양수로, 신규확진자가 증가할수록 편의점 식품 음료 등 가공 성장률이 높아짐을 알 수 있었다. 마지막으로 독립변수에 의해 종속변수가 설명되는 설명력은 34.7%임을 확인할 수 있었다.";
